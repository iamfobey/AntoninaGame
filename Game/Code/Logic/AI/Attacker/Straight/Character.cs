...
using Game.Logic.AI.Attacker.Straight.FSM;
using Game.Utils;
using Godot;

namespace Game.Logic.AI.Attacker.Straight
{
    [GameSerializable]
    public partial class Character : Core.AI.Attacker.Character
    {
        ...
        public override void _PhysicsProcessGame(double delta)
        {
            ...
            if (IsNavFinished() && PoisonedComponent.IsPoisoned && !FSM.IsCurrentStateEqual<AttackState>())
            {
                if (this.DistanceToPlayer() < MaxDistanceToAttackPlayer)
                    ...
                    NavigationAgent.TargetPosition = NavigationRegion.GetRandomPointRadius(Player.Character.Instance.GlobalPosition, AttackRadius.X,
                        AttackRadius.Y);
            }
        ...
        [ExportGroup("Sockets")]
        [Export]
        public Node2D ProjectileSocket;

        [ExportCategory("Parameters")]
        [ExportGroup("Character Logic")]
        [Export]
        public float MaxDistanceToAttackPlayer = 1600.0f;
        [Export]
